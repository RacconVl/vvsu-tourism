    ...
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: WHITE, minHeight: "100vh" }}>
      ...
      <nav style={{ background: WHITE, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 48px", height: 68, borderBottom: `3px solid ${BLACK}` }}>
        ...
        <div style={{ display: "flex", gap: 0, alignItems: "center" }}>
          ...
          <button style={{ marginLeft: 20, background: PINK, color: WHITE, border: "none", borderRadius: 0, padding: "12px 28px", fontWeight: 800, fontSize: 13, cursor: "pointer", letterSpacing: 1, textTransform: "uppercase" }}>
            Войти
          </button>
        </div>
      </nav>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", minHeight: 520 }}>
        ...
        <div style={{ padding: "64px 48px", display: "flex", flexDirection: "column", justifyContent: "space-between", borderRight: `4px solid ${BLACK}` }}>
          <div>
            <div style={{ display: "inline-block", background: PINK, color: WHITE, fontWeight: 800, fontSize: 11, letterSpacing: 3, textTransform: "uppercase", padding: "6px 14px", marginBottom: 36 }}>
              Владивосток · 2024
            </div>
        ...
        <div style={{ display: "grid", gridTemplateRows: "1fr 1fr" }}>
          ...
          {/* Bottom: PINK */}
          <div style={{ background: PINK, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)" }}>
        {[
          ...
          { num: "03", title: "Гостиничное дело", desc: "Управление отелями 5★ в Приморье и АТР", color: PINK, textColor: WHITE },
        ].map((p, i) => (
          <div key={i} style={{
            background: p.color,
            padding: "52px 40px",
      ...
      <div style={{ background: BLACK, padding: "80px 48px", borderTop: `4px solid ${BLACK}`, display: "flex", alignItems: "center", gap: 80 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: WHITE, fontSize: 36, fontWeight: 700, lineHeight: 1.3, letterSpacing: -1 }}>
            ...
            <span style={{ color: PINK }}>создаёт профессионалов,</span><br/>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderTop: `4px solid ${BLACK}` }}>
        ...
        {/* Right: PINK */}
        <div style={{ background: PINK, padding: "60px 48px", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", bottom: -80, right: -80, width: 320, height: 320, borderRadius: "50%", background: "rgba(0,0,0,0.1)" }} />
          <div style={{ color: "rgba(255,255,255,0.7)", fontSize: 11, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", marginBottom: 20 }}>→ Карта путешествий</div>
          <div style={{ color: WHITE, fontSize: 44, fontWeight: 900, lineHeight: 1.1, letterSpacing: -2, marginBottom: 20 }}>
          ...
          {["⚓ Порт отплытия", "🗺️ Архипелаг знаний", "⛵ Открытый океан", "🐋 Марианская впадина", "🌅 Тихоокеанский горизонт"].map((stage, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: i <= 1 ? WHITE : "rgba(255,255,255,0.3)" }} />
              <span style={{ color: i <= 1 ? WHITE : "rgba(255,255,255,0.55)", fontSize: 12, fontWeight: i <= 1 ? 700 : 500 }}>{stage}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* FOOTER */}
      <div style={{ background: BLACK, padding: "28px 48px", display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: `4px solid ${PINK}` }}>
